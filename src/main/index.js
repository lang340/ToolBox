const { app, BrowserWindow, ipcMain, clipboard: electronClipboard, nativeImage } = require('electron');
const path = require('path');
const fs = require('fs');
const { initDB, db, query, run, closeDB, saveToFile } = require('./db-service');
...
function registerClipboardHandlers() {
  ...
  ipcMain.handle('clipboard:clear', () => {
    ...
    images.forEach((row) => deleteImage('clipboard', row.content));
...
function startClipboardWatch() {
  ...
  lastClipboardImageHash = img.isEmpty() ? '' : img.toBitmap().length.toString();
  ...
  clipboardIntervalId = setInterval(() => {
    ...
    if (!currentImage.isEmpty()) {
      const hash = currentImage.toBitmap().length.toString();
...
function registerTaskHandlers() {
  ...
  ipcMain.handle('task:update', (_event, id, fields) => {
    const sets = [];
    const values = [];
  ...
  ipcMain.handle('task:delete', (_event, id) => {
    run('DELETE FROM task_completions WHERE task_id = ?', [id]);
    run('DELETE FROM tasks WHERE id = ?', [id]);
    return { success: true };
  });

  ipcMain.handle('task:get-by-date', (_event, date) => {
  ...
  ipcMain.handle('task:get-by-month', (_event, year, month) => {
    const allTasks = query('SELECT * FROM tasks ORDER BY created_at DESC');
  ...
  ipcMain.handle('task:toggle-complete', (_event, id, date) => {
...
function isTaskActiveOnDate(task, dateStr) {
  ...
  if (task.end_date && new Date(task.end_date) < checkDate) return false;
  ...
  switch (task.repeat_type) {
    ...
    case 'days': {
      const meta = JSON.parse(task.repeat_meta || '{}');
...
function registerDiaryHandlers() {
  ipcMain.handle('diary:get', (_event, date) => {
    const rows = query('SELECT * FROM diaries WHERE date = ?', [date]);
    return rows.length > 0 ? rows[0] : null;
  });

  ipcMain.handle('diary:save', (_event, date, content) => {
    const existing = query('SELECT id FROM diaries WHERE date = ?', [date]);
    if (existing.length > 0) {
      run("UPDATE diaries SET content = ?, updated_at = datetime('now', 'localtime') WHERE date = ?", [content, date]);
    } else {
      run('INSERT INTO diaries (date, content) VALUES (?, ?)', [date, content]);
    }
    return { success: true };
  });

  ipcMain.handle('diary:get-list', (_event, month) => {
    return query('SELECT date FROM diaries WHERE date LIKE ? ORDER BY date DESC', [`${month}%`]);
  });

  ipcMain.handle('diary:upload-image', (_event, data, ext) => {
    const buffer = Buffer.from(data, 'base64');
    const filename = saveImage('diary', buffer, ext);
    return filename;
  });
}

function registerPomodoroHandlers() {
  ipcMain.handle('pomodoro:save-record', (_event, duration, type) => {
    const result = run('INSERT INTO pomodoro_records (duration, type) VALUES (?, ?)', [duration, type]);
    return { id: result.lastInsertRowid };
  });

  ipcMain.handle('pomodoro:get-today', () => {
    const rows = query(
